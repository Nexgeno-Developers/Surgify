...
https://www.youtube.com/shorts/ibNe6LcbeF4 */
  youtubeVideos: any[] = [
    {
      url: 'xZMbj4GXAoI',
      desciption:
        'Mr Anil is back to his healthy & amazing life after his succesful surgery with @surgify',
    },
    {
      url: 'tjRTxKdIqic',
      description:
        'Mr. sagar borigam experiencing a kidney stone surgery with surgify',
    },
    { url: 'iQ3bV0QcWVA', description: 'Mallesham Borigam at Surgify' },
    {
      url: 'ibNe6LcbeF4',
      description: 'Varsha rajgole with a successful hernia surgery at SURGIFY',
    },
  ];
  testimonialData = [
    {
  ...
  constructor(
    ...
    this.onResize();
    this.titleService.setTitle('Surgify: Best Piles & Fistula Clinic in India');
    this.meta.updateTag({ content: '' }, "name='keywords'");
    this.meta.updateTag(
      {
        content:
          'Your well-being is our priority. Book an appointment at Surgify Piles & Fistula Clinic for compassionate care and support on your journey to recovery.',
      },
      "name='description'"
    );
        ...
        if (specialists && specialists.length > 0) {
          specialists.forEach((sp: any, spInd: number) => {
            this.surgeriesService
              .getAllSurgeriesBySpecialization(sp.id)
              .subscribe(
                (surgeries) => {
                  this.surgeries.push({
                  ...
                  if (spInd == specialists.length - 1) {
                    ...
                    this.allSurgeries.forEach((sur, ind) => {
                      let findIt = this.randomSurgeries.find(
                        (s) => s.name.toLowerCase() == sur.name.toLowerCase()
                      );
                      if (!findIt) {
                    ...
                    if (
                      this.allSurgeries.length == this.randomSurgeries.length
                    ) {
                      this.surgeriesPerfomLoading = false;
                      this.setRandom();
                    }
  ...
  openBookDialog(specialization?: any) {
    const mod = this.modalService.open(BookingComponent, {
      windowClass: 'appointment-modal ',
      modalDialogClass: 'modal-dialog-centered',
    });
    if (specialization) {
      let found = this.allSurgeries.find(
        (sur) => sur.name.toLowerCase() == specialization.toLowerCase()
      );
  ...
  setRandom() {
    ...
    list.forEach((sur) => {
      let index = this.randomSurgeries.findIndex(
        (s) => s.name.toLowerCase() == sur.toLowerCase()
      );
  ...
  goToDetail(surgery: any) {
    let speciality = this.specialities.find(
      (sp) => sp?.id === surgery.specializationId
    );

    let findIt = this.randomSurgeries.find(
      (s) => s.name.toLowerCase() == surgery.name.toLowerCase()
    );
  ...
  getAllHospitals() {
    this.hospitalService.getAllHospitals(6).subscribe(
      (list) => {
  ...
  openVideo(videoId: any, content: any) {
    ...
    this.modalService
      .open(content, {
        modalDialogClass: 'modal-dialog-centered',
        windowClass: 'loginPup videoPup',
      })
      .shown.subscribe({
        next: (s) => {
          let ele = document
            .getElementById('videoP')
            ?.querySelector('iframe')
            ?.classList.add('ngVideo');
        },
        error: (err) => {},
      });
  }
